public class MigrationSimulator {
    ...
    public void initBirds() {
        // TODO Student implements, similar to initAnimals()
    ...
    public void simulateBirdsBears() {
        // TODO Student implements, similar to simulateAnimals()
    	for (int iter = 0; iter < niter; iter++) {
            for (int i = 0; i < birds.length; i++) {
    ...
    public int[] getBirdBearCount(Site site) throws IllegalAnimalException {
        // TODO Student implements, similar to getAnimalCount()
    	int cbirds = 0;
    	int cbears = 0;
        String status;
        for (Animal animal : site.getAnimals()) {
            status = animal.getStatus();
            if (status.equals("ALIVE") && animal instanceof Bird) {
                cbirds++;
            }
            else if (status.equals("ALIVE") && animal instanceof Bear) {
            	cbears++;
            }
            else throw new IllegalAnimalException("An unknown animal is found in the site");
        }
        int arr[] = {cbirds,cbears};
        return arr;
    }
